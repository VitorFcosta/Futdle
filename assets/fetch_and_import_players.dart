...
class FetchAndImportPlayers {
  static Future<void> run() async {
    ...
    AppLogger.info(
      'Iniciando importação via football-data.org (Aguarde ~15 minutos)...',
    );
      ...
      try {
        ...
        for (final team in teams) {
          ...
          for (final player in squad) {
            ...
            if (!seenNames.contains(playerName)) {
              ...
              batch.set(docRef, {
                ...
                'position':
                    player['position'], // Ex: "Offence", "Midfield", "Defence"
              });
          ...
          if (batchCount > 0) {
            await batch.commit();
            AppLogger.info(
              '+$batchCount jogadores salvos. Total atual: $totalImported',
            );
          }
    ...
    AppLogger.info(
      '🎉 Concluído! O banco de dados do seu Futdle está 100% atualizado.',
    );
  }
